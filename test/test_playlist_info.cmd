...
CALL CallPipe 'playlist' dir'\data\list.lst'
CALL Assert 'TRANSLATE(RESULT)', '= "'dirurl'/DATA/LIST.LST"'

CALL CallPipe 'info format 'dir'\data\list.lst'
reply = result
CALL Parse result
CALL Assert 'data.filesize', '= 128', reply
CALL Assert 'POS("playlist", data.flags)', '\= 0', reply
/*CALL Assert 'POS("writable", data.flags)', '\= 0', data.flags*/
CALL Assert 'TRANSLATE(LEFT(data.decoder, 8))', '= "PLIST123"', reply
CALL Assert 'data.subitems', '= 4', reply

CALL CallPipe 'info playlist 'dir'\data\list.lst'
reply = result
CALL Parse result
CALL Assert 'data.songs', '= 3', reply
CALL Assert 'data.lists', '= 1', reply
