...
#define SAFE_NTOM /* Do not depend on mpg123_off_t*mpg123_off_t with big values still being in the range... */
...
/*
	...
	it is more about avoiding multiplication of possibly big sample offsets (a 32bit mpg123_off_t could overflow too easily).
*/

unsigned long ntom_val(mpg123_handle *fr, mpg123_off_t frame)
{
	mpg123_off_t ntm;
...
#endif
	mpg123_off_t soff = 0;
	mpg123_off_t ntm = ntom_val(fr,0);
#ifdef SAFE_NTOM
	if(frame <= 0) return 0;
	for(f=0; f<frame; ++f)
	{
...
#else
	soff = (ntm + frame*(mpg123_off_t)spf(fr)*(mpg123_off_t)fr->ntom_step)/(mpg123_off_t)NTOM_MUL;
#endif
	return soff;
}
...
#ifdef SAFE_NTOM
	{
		mpg123_off_t block = spf(fr);
		if(ins <= 0) return 0;
		do
		{
			mpg123_off_t nowblock = ins > block ? block : ins;
...
#else
	/* Beware of overflows: when mpg123_off_t is 32bits, the multiplication blows too easily.
	   Of course, it blows for 64bits, too, in theory, but that's for _really_ large files. */
	soff = ((mpg123_off_t)ntm + (mpg123_off_t)ins*(mpg123_off_t)fr->ntom_step)/(mpg123_off_t)NTOM_MUL;
#endif
	return soff;
}
...
{
	mpg123_off_t ioff = 0; /* frames or samples */
	mpg123_off_t ntm = ntom_val(fr,0);
#ifdef SAFE_NTOM
	if(soff <= 0) return 0;
	for(ioff=0; 1; ++ioff)
	...
	return ioff;
#else
	ioff = (soff*(mpg123_off_t)NTOM_MUL-ntm)/(mpg123_off_t)fr->ntom_step;
	return ioff/(mpg123_off_t)spf(fr);
#endif
}
