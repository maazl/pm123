/*
	...
	mpg123_off_t /= long, and clients still insisting on defining _FILE_OFFSET_BITS.
	...
	Note that the mpg123 header is _not_ used here to avoid definition with whacky mpg123_off_t.
	The aliases are always about arguments of native alias_t type. This can be mpg123_off_t, but
	on Linux/x86, this is long int. The mpg123_off_t declarations in mpg123.h confuse things,
...
if(/^\s*EXPORT\s+(\S+)\s+(mpg123_\S+)\((.*)\);\s*$/)
{
	...
	$type =~ s/mpg123_off_t/lfs_alias_t/g;
	my @nargs = ();
	$args =~ s/mpg123_off_t/lfs_alias_t/g;
