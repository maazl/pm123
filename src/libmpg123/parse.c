			...
			if(lame_type)
			{
				...
				if(fr->bsbuf[lame_offset] != 0)
				{
					...
					*/
					...
					if(fr->p.flags & MPG123_GAPLESS)
					{
						mpg123_off_t skipbegin = ((((int) fr->bsbuf[lame_offset]) << 4) | (((int) fr->bsbuf[lame_offset+1]) >> 4));
						mpg123_off_t skipend = (((((int) fr->bsbuf[lame_offset+1]) << 8) | (((int) fr->bsbuf[lame_offset+2]))) & 0xfff);
