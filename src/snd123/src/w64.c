...
{   int subformat, error, blockalign = 0, framesperblock = 0 ;
    ...
    {   if ((error = w64_read_header (psf, &blockalign, &framesperblock)) != 0)
            return error ;
        } ;
    ...
    {   if (psf->is_pipe)
        ...
        if ((error = w64_write_header (psf, SF_FALSE)) != 0)
            return error ;
    ...
    {   /* Read the 4 byte marker and jump 12 bytes. */
        ...
        {   case riff_HASH16 :
            ...
            case fmt_HASH16 :
                    ...
                    if ((error = wav_w64_read_fmt_chunk (psf, &wav_fmt, (int) chunk_size)) != 0)
                        return error ;
