...
{   int     subformat, error, endian ;
    ...
    {   if ((error = paf_read_header (psf)) != 0)
            return error ;
        } ;
    ...
    {   if ((psf->sf.format & SF_FORMAT_TYPEMASK) != SF_FORMAT_PAF)
        ...
        if ((error = paf_write_header (psf, SF_FALSE)) != 0)
            return error ;
...
#else
    ppaf24->samples     = (int*) ((char*) ppaf24 + sizeof(*ppaf24)) ;
    ppaf24->block       = (unsigned char*) (ppaf24->samples + PAF24_SAMPLES_PER_BLOCK * ppaf24->channels) ;
#endif
...
{   int             k, channel ;
    ...
    if ((k = psf_fread (ppaf24->block, 1, ppaf24->blocksize, psf)) != ppaf24->blocksize)
        ...
        if (psf->endian == SF_ENDIAN_BIG)
        {
            ...
            #else
            endswap_int_array ((int*) ((char*) ppaf24 + sizeof(*ppaf24)), 8 * ppaf24->channels) ;
            #endif
            } ;
