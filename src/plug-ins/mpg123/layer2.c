...
void init_layer2(int down_samp)
{
  ...
  for(k=0;k<27;k++)
  {
    ...
    if(mmx_enable && !down_samp)
    {
      for(j=3,i=0;i<63;i++,j--)
        *table++ = 16384.0 * m * pow(2.0,(double) j / 3.0);
    }
    else
    {
      for(j=3,i=0;i<63;i++,j--)
...
int do_layer2(DECODER_STRUCT *w, struct frame *fr)
{
  int clip=0;
  int i,j;
  int stereo = fr->stereo;
  ...
  _control87(EM_OVERFLOW | EM_UNDERFLOW | EM_ZERODIVIDE | EM_INEXACT |
             EM_DENORMAL, MCW_EM);
