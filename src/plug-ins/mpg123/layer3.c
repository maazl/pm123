 ...
 */
static int III_get_side_info_1(DECODER_STRUCT *w, struct III_sideinfo *si,int stereo,
 ...
 */
static int III_get_side_info_2(DECODER_STRUCT *w,struct III_sideinfo *si,int stereo,
 int ms_stereo,long sfreq,int single)
{
   int ch;
   int powdiff = (single == 3) ? 4 : 0;
...
static int III_dequantize_sample_ms(DECODER_STRUCT *w,real xr[2][SBLIMIT][SSLIMIT],int *scf,
   struct gr_info_s *gr_info,int sfreq,int part2bits)
{
  int shift = 1 + gr_info->scalefac_scale;
 ...
 * main layer3 handler
 */
int do_layer3(DECODER_STRUCT *w, struct frame *fr)
{
  int gr, ch, ss,clip=0;
  ...
  for (gr=0;gr<granules;gr++)
  {
...

    {
      ...
      if(III_dequantize_sample(w,hybridIn[0], scalefacs[0],gr_info,sfreq,part2bits))
        return clip;
    }
    if(stereo == 2) {
      ...
      if(ms_stereo) {
        if(III_dequantize_sample_ms(w,hybridIn,scalefacs[1],gr_info,sfreq,part2bits))
          return clip;
      }
      else {
        if(III_dequantize_sample(w,hybridIn[1],scalefacs[1],gr_info,sfreq,part2bits))
          return clip;
      }
...
// dink stuff
      if(gr_info->maxb < 1 || gr_info->maxb > SBLIMIT)
          return 0; // anti-crash (dink)
      do_mp3eq((real*)hybridIn[ch],ch);
    ...
    for(ss=0;ss<SSLIMIT;ss++) {
      ...
      if(pcm_point >= w->audio_buffersize)
         if(!output_flush(w,fr))
            return -1;
    }
  }
