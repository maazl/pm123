...
static void
v_writestring( oggpack_buffer* o, char* s, int len )
{
  while( len-- ) {
    oggpack_write( o, *s++, 8 );
  }
...
static int
commentheader_out( vorbis_comment* vc, char* vendor, ogg_packet* op )
{
  oggpack_buffer opb;
  ...
  v_writestring( &opb, "vorbis", 6 );

  /* vendor */
  oggpack_write( &opb, strlen( vendor ), 32 );
  v_writestring( &opb, vendor, strlen( vendor ));
  ...
  if( vc->comments )
  {
    ...
    for( i = 0; i < vc->comments; i++ ) {
      if( vc->user_comments[i] ) {
        ...
        v_writestring( &opb, vc->user_comments[i], vc->comment_lengths[i] );
      } else {
...
static int
fetch_next_packet( vcedit_state* s, ogg_packet* p, ogg_page* page )
{
  int   result;
  char* buffer;
  int   bytes;
  ...
  if( result > 0 ) {
    ...
    return fetch_next_packet( s, p, page );
  }
}
...
vcedit_write( vcedit_state* state, void* out )
{
  ...
  commentheader_out( state->vc, state->vendor, &header_comments );
  ...
  while( fetch_next_packet( state, &op, &ogin ))
  {
    int size = blocksize( state, &op );
    granpos += size;
