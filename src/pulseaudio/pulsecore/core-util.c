...
int pa_make_realtime(int rtprio) {
...
#elif defined(_POSIX_PRIORITY_SCHEDULING) && _POSIX_PRIORITY_SCHEDULING != -1
    int p;
    ...
    errno = EPERM;
#elif defined(OS_IS_OS2)
    APIRET rc = DosSetPriority(PRTYS_THREAD, PRTYC_TIMECRITICAL, -rtprio & 0xff, 0);
    if (rc != 0) {
        pa_log_warn("DosSetPriority time critical failed: %lu", rc);
        errno = EPERM;
        return -1;
    }

    pa_log_info("Successfully gained time critical priority class.");
    return 0;
#else
    errno = ENOTSUP;
#endif
...
static priorityentry priority_backup[128] = { { 0, 0 } };
#endif

/* Raise the priority temporarily to avoid priority inversion
 * before the mutex acquisition. */
void pa_temp_raise_priority(int nice_level)
{
#ifdef OS_IS_OS2
    PTIB tib;
    priorityentry* pe;
    DosGetInfoBlocks(&tib, NULL);
    pe = priority_backup + tib->tib_ptib2->tib2_ultid;
    if (++pe->priority_depth == 1)
    {   int pri = tib->tib_ptib2->tib2_ulpri;
        if (pri >= -nice_level) // Already high
          pri = INT_MIN; // no-op marker
        else
          pa_raise_priority(nice_level);
        pe->priority_before_lock = pri;
    }
    pa_log_debug("pa_temp_raise_priority %u, %x, %x, %i\n", tib->tib_ptib2->tib2_ultid, tib->tib_ptib2->tib2_ulpri, pe->priority_before_lock, pe->priority_depth);
#endif
}

/* restore original thread priority after mutex release */
void pa_temp_restore_priority(void)
{
#ifdef OS_IS_OS2
    PTIB tib;
    priorityentry* pe;
    DosGetInfoBlocks(&tib, NULL);
    pe = priority_backup + tib->tib_ptib2->tib2_ultid;
    if (--pe->priority_depth == 0 && pe->priority_before_lock != INT_MIN)
    {   // Restore priority before lock, but only if we are at the top level.
        pa_raise_priority(-pe->priority_before_lock);
    }
    pa_log_debug("pa_temp_restore_priority %u, %x, %x, %i\n", tib->tib_ptib2->tib2_ultid, tib->tib_ptib2->tib2_ulpri, pe->priority_before_lock, pe->priority_depth);
#endif
}
...
int pa_match(const char *expr, const char *v) {
    ...
    return r;
#else
    /* Hmm, we have no regex implementation */
    errno = ENOTSUP;
    return -1;
#endif
}
...
#if !defined(OS_IS_WIN32) && !defined(OS_IS_OS2)
    struct rlimit rl;
    int maxfd, fd;
...
#endif  /* !defined(OS_IS_WIN32) && !defined(OS_IS_OS2) */
...
#elif defined(OS_IS_OS2)
    ULONG version[3];
    DosQuerySysInfo(QSV_VERSION_MAJOR, QSV_VERSION_REVISION, &version, sizeof version);
    return pa_sprintf_malloc("OS/2 %lu.%lu (%lu)", version[0], version[1], version[2]);
#else
    return pa_xstrdup("unknown");
#endif
}
