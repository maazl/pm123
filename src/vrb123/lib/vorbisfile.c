...
fread_wrap(void *buffer,size_t size,size_t count,void *stream) {
...
/* if, eg, 64 bit stdio is configured by default, this will build with
   fseek64 */
static int DLLENTRY
_fseek64_wrap(void *stream,ogg_int64_t off,int whence){
  if(stream==NULL)return(-1);
  return fseek((FILE*)stream,off,whence);
}
...
int ov_open(FILE *f,OggVorbis_File *vf,char *initial,long ibytes){
  ov_callbacks callbacks = { fread_wrap, _fseek64_wrap, fclose_wrap, ftell_wrap };
...
int ov_pcm_seek_page(OggVorbis_File *vf,ogg_int64_t pos){
  ...
  /* new search algorithm by HB (Nicholas Vinen) */
  {
    ...
    while(begin<end){
      ...
      DEBUGLOG(("ov_pcm_seek_page - %li,%li %li,%li %li %li -> %li\n", begin, end, begintime, endtime, target, best, bisect));
...
int ov_pcm_seek(OggVorbis_File *vf,ogg_int64_t pos){
  ...
  if(ret<0)return(ret);
  if((ret=_make_decode_ready(vf))!=0)return ret;
...
static int _ov_64_seek_lap(OggVorbis_File *vf,ogg_int64_t pos,
                           int DLLENTRYP(localseek)(OggVorbis_File *,ogg_int64_t)){
  vorbis_info *vi;
  float **lappcm;
  float **pcm;
  float *w1,*w2;
...
static int _ov_d_seek_lap(OggVorbis_File *vf,double pos,
                           int DLLENTRYP(localseek)(OggVorbis_File *,double)){
  vorbis_info *vi;
  float **lappcm;
  float **pcm;
  float *w1,*w2;
